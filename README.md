...
File `services/storageService.js` là nơi tập trung mọi thao tác lưu trữ:

| Nhóm  | Hàm                                         |
|-------|---------------------------------------------|
| Auth  | `saveUser`, `getUser`, `removeUser`         |
| Cart  | `getCart`, `saveCart`, `removeCart`         |
| Order | `getOrders`, `saveOrder`                    |

Tất cả đều dùng `async/await` + `try/catch` và lưu dưới dạng JSON
...
Đặt các file ảnh vào thư mục `docs/` (hoặc paste trực tiếp ở đây khi nộp bài):
...
Link video (2–5 phút): **<thay bằng link YouTube / Drive của bạn>**

Nội dung video:
- Giới thiệu: "Đây là bài thực hành AsyncStorage của em — Nguyễn Thị Khánh
  Linh, MSSV 23810310394".
- Demo: Login → kill app → mở lại (auto login).
- Demo: Thêm giỏ → kill app → mở lại vẫn còn.
- Demo: Checkout → xem danh sách đơn hàng → reload vẫn còn.
- Header/Account có hiển thị tên sinh viên.
...
## 7. Gợi ý commit history (≥5 commit)

```
feat: setup storage service with async/await and try/catch
feat: login with asyncstorage and auto login on app start
feat: cart persistence with quantity update and remove
feat: save order on checkout and list on orders screen
fix: order schema aligned with orders screen (createdAt, items, total)
docs: add README with student info, screenshots and theory answers
```
